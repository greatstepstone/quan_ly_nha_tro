...
final roomInvoicesProvider = StreamProvider.family<List<Invoice>, String>((
  ref,
  roomId,
) {
...
final invoiceFilterStatusProvider = StateProvider<InvoiceStatus?>(
  (ref) => null,
);
final invoiceSelectedMonthProvider = StateProvider<DateTime>(
  (ref) => DateTime.now(),
);
...
final filteredInvoicesProvider = FutureProvider<List<Invoice>>((ref) async {
  ...
  return allInvoices.where((inv) {
    ...
    if (inv.createdAt != null) {
      ...
      matchesMonth =
          date.month == selectedMonth.month && date.year == selectedMonth.year;
    }
    ...
    return invs
        .where(
          (i) =>
              ...
              i.status != InvoiceStatus.notCreated,
        )
        .fold(0.0, (sum, i) => sum + i.totalAmount);
  });
});
