...
final userDaoProvider = Provider<UserDao>(
  (ref) => ref.watch(databaseProvider).userDao,
);
final propertyDaoProvider = Provider<PropertyDao>(
  (ref) => ref.watch(databaseProvider).propertyDao,
);
final roomDaoProvider = Provider<RoomDao>(
  (ref) => ref.watch(databaseProvider).roomDao,
);
final tenantDaoProvider = Provider<TenantDao>(
  (ref) => ref.watch(databaseProvider).tenantDao,
);
final meterReadingDaoProvider = Provider<MeterReadingDao>(
  (ref) => ref.watch(databaseProvider).meterReadingDao,
);
final invoiceDaoProvider = Provider<InvoiceDao>(
  (ref) => ref.watch(databaseProvider).invoiceDao,
);
final serviceDaoProvider = Provider<ServiceDao>(
  (ref) => ref.watch(databaseProvider).serviceDao,
);

final roomMemberDaoProvider = Provider<RoomMemberDao>(
  (ref) => ref.watch(databaseProvider).roomMemberDao,
);
