...
class InvoiceDao extends DatabaseAccessor<AppDatabase> with _$InvoiceDaoMixin {
  ...
  Future<List<Invoice>> getAllInvoices() =>
      (select(invoices)..where((t) => t.isDeleted.equals(false))).get();
  Stream<List<Invoice>> watchAllInvoices() =>
      (select(invoices)..where((t) => t.isDeleted.equals(false))).watch();
  ...
  Future<int> insertInvoice(Insertable<Invoice> invoice) =>
      into(invoices).insertOnConflictUpdate(invoice);
  Future<bool> updateInvoice(Insertable<Invoice> invoice) =>
      update(invoices).replace(invoice);

  Future<int> deleteInvoice(String id) => (update(invoices)..where(
    (t) => t.id.equals(id),
  )).write(const InvoicesCompanion(isDeleted: Value(true)));
  Future<int> hardDeleteInvoice(String id) =>
  ...
  Future<List<Invoice>> getDeletedInvoices() =>
      (select(invoices)..where((t) => t.isDeleted.equals(true))).get();
}
