...
class ContractDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<Contract?> getContractById(String id) =>
      (select(contracts)..where(
        (c) => c.id.equals(id) & c.isDeleted.equals(false),
      )).getSingleOrNull();

  Future<List<Contract>> getContractsByRoom(String roomId) =>
      (select(contracts)..where(
        (c) => c.roomId.equals(roomId) & c.isDeleted.equals(false),
      )).get();
  ...
  Future<Contract?> getActiveContractByRoom(String roomId) =>
      (select(contracts)..where(
        (c) =>
            ...
            c.status.equalsValue(ContractStatus.active),
      )).getSingleOrNull();
  ...
  Future<List<Contract>> getContractsByProperty(String propertyId) =>
      (select(contracts)..where(
        (c) => c.propertyId.equals(propertyId) & c.isDeleted.equals(false),
      )).get();
  ...
  Future<int> deleteContract(String id) => (update(contracts)..where(
    (c) => c.id.equals(id),
  )).write(const ContractsCompanion(isDeleted: Value(true)));
  ...
  Future<List<Contract>> getUnsyncedContracts() =>
      (select(contracts)..where(
        (c) => c.isSynced.equals(false) & c.isDeleted.equals(false),
      )).get();
