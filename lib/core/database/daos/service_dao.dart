...
class ServiceDao extends DatabaseAccessor<AppDatabase> with _$ServiceDaoMixin {
  ...
  Future<int> insertService(Insertable<Service> service) =>
      into(services).insertOnConflictUpdate(service);
  Future<bool> updateService(Insertable<Service> service) =>
      update(services).replace(service);

  // Soft delete
  Future<int> deleteService(String id) => (update(services)..where(
    (t) => t.id.equals(id),
  )).write(const ServicesCompanion(isDeleted: Value(true)));

  // Hard delete
  Future<int> hardDeleteService(String id) =>
