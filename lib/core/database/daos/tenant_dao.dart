...
class TenantDao extends DatabaseAccessor<AppDatabase> with _$TenantDaoMixin {
  ...
  Future<List<Tenant>> getAllTenants() =>
      (select(tenants)..where((t) => t.isDeleted.equals(false))).get();
  Stream<List<Tenant>> watchAllTenants() =>
      (select(tenants)..where((t) => t.isDeleted.equals(false))).watch();
  ...
  Future<List<Tenant>> getTenantsByRoom(String roomId) =>
      (select(tenants)..where(
        (t) => t.roomId.equals(roomId) & t.isDeleted.equals(false),
      )).get();
  Future<int> insertTenant(Insertable<Tenant> tenant) =>
      into(tenants).insertOnConflictUpdate(tenant);
  Future<bool> updateTenant(Insertable<Tenant> tenant) =>
      update(tenants).replace(tenant);

  Future<int> deleteTenant(String id) => (update(tenants)..where(
    (t) => t.id.equals(id),
  )).write(const TenantsCompanion(isDeleted: Value(true)));
  Future<int> hardDeleteTenant(String id) =>
      (delete(tenants)..where((t) => t.id.equals(id))).go();
  Future<List<Tenant>> getUnsyncedTenants() =>
      (select(tenants)..where(
        (t) => t.isSynced.equals(false) & t.isDeleted.equals(false),
      )).get();
  Future<List<Tenant>> getDeletedTenants() =>
      (select(tenants)..where((t) => t.isDeleted.equals(true))).get();
}
