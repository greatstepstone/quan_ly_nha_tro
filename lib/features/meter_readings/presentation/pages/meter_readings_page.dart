...
class _MeterReadingsPageState extends ConsumerState<MeterReadingsPage> {
  ...
  TextEditingController _getElectricCtrl(String id) =>
      _electricControllers.putIfAbsent(id, () => TextEditingController());
  TextEditingController _getWaterCtrl(String id) =>
      _waterControllers.putIfAbsent(id, () => TextEditingController());

  @override
  void dispose() {
    for (var c in _electricControllers.values) {
      c.dispose();
    }
    for (var c in _waterControllers.values) {
      c.dispose();
    }
    super.dispose();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: propertiesAsync.when(
        data:
            (properties) => roomsAsync.when(
              data:
                  (rooms) => readingsAsync.when(
                    data: (allReadings) {
                      ...
                      final readings =
                          allReadings.where((MeterReading r) {
                            final room = rooms.firstWhere(
                              (Room rm) => rm.id == r.roomId,
                              orElse: () => rooms.first,
                            );
                            if (currentPropertyId != null &&
                                room.propertyId != currentPropertyId)
                              return false;
                    ...
                    error:
                        (err, _) => Center(child: Text('Lỗi tải chỉ số: $err')),
                  ),
          ...
          child: ElevatedButton.icon(
            ...
            onPressed: () {
              ScaffoldMessenger.of(
                context,
              ).showSnackBar(const SnackBar(content: Text('Đã lưu chỉ số!')));
            },
