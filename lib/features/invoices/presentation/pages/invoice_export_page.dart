...
class _InvoiceExportPageState extends State<InvoiceExportPage> {
  ...
  double get _electricCost => (widget.electricNew - widget.electricOld).clamp(0, 99999) * _electricPrice;
  double get _waterCost => (widget.waterNew - widget.waterOld).clamp(0, 99999) * _waterPrice;
  double get _rentPrice => widget.room.rentPrice;
  double get _servicesCost => widget.services.fold(0.0, (sum, s) => sum + s.price);
  double get _total => _electricCost + _waterCost + _rentPrice + _servicesCost;

  Future<void> _downloadAsImage() async {
    setState(() => _isExporting = true);
    try {
      final Uint8List? bytes = await _screenshotController.capture(pixelRatio: 2.5);
      if (bytes == null) return;

      final fileName = 'hoa_don_${widget.room.name}_${widget.invoice.month.replaceAll('/', '-')}.png';
      ...
      if (mounted) {
        final msg = kIsWeb ? 'Đã tải xuống hóa đơn dạng ảnh!' : 'Đã mở danh sách chia sẻ!';
          ...
          build: (pw.Context context) {
            return pw.Column(
              ...
              children: [
                  ...
                  child: pw.Row(
                    ...
                    children: [
                      pw.Column(
                        crossAxisAlignment: pw.CrossAxisAlignment.start,
                        children: [
                          pw.Text('HÓA ĐƠN TIỀN PHÒNG',
                            style: pw.TextStyle(font: fontBold, fontSize: 18, color: PdfColors.white)),
                          pw.SizedBox(height: 4),
                          pw.Text('Tháng ${widget.invoice.month}',
                            style: pw.TextStyle(font: fontData, fontSize: 13, color: PdfColors.white)),
                        ],
                      ),
                      pw.Text(_fmt(_total),
                ...
                // Room info
                pw.Row(children: [
                  pw.Expanded(child: _pdfInfoBox(fontData, fontBold, 'PHÒNG', widget.room.name)),
                  pw.SizedBox(width: 12),
                  pw.Expanded(child: _pdfInfoBox(fontData, fontBold, 'NHÀ TRỌ', widget.property?.name ?? '-')),
                  pw.SizedBox(width: 12),
                  pw.Expanded(child: _pdfInfoBox(fontData, fontBold, 'KHÁCH THUÊ', widget.tenant?.name ?? '-')),
                ]),
                pw.SizedBox(height: 20),
                ...
                _pdfSection(fontData, fontBold, 'CHI TIẾT HÓA ĐƠN', [
                  ...
                  _pdfRow(fontData, fontBold,
                    ...
                    _waterCost),
                  ...widget.services.map((s) => _pdfRow(fontData, fontBold, s.name, s.price)),
                ]),
  ...
  pw.Widget _pdfInfoBox(pw.Font fontData, pw.Font fontBold, String label, String value) {
    return pw.Container(
      ...
      child: pw.Column(
        crossAxisAlignment: pw.CrossAxisAlignment.start,
        children: [
          pw.Text(label, style: pw.TextStyle(font: fontBold, fontSize: 9, color: PdfColors.grey)),
          pw.SizedBox(height: 4),
  ...
  pw.Widget _pdfSection(pw.Font fontData, pw.Font fontBold, String title, List<pw.Widget> rows) {
    return pw.Container(
      padding: const pw.EdgeInsets.all(16),
      decoration: pw.BoxDecoration(
      ...
      child: pw.Column(
        crossAxisAlignment: pw.CrossAxisAlignment.start,
        children: [
          pw.Text(title, style: pw.TextStyle(font: fontBold, fontSize: 11, color: PdfColors.grey700)),
  ...
  pw.Widget _pdfRow(pw.Font fontData, pw.Font fontBold, String label, double value) {
    return pw.Padding(
      ...
      child: pw.Row(
        ...
        children: [
          ...
          pw.Text(_fmt(value), style: pw.TextStyle(font: fontBold, fontSize: 12)),
        ],
      ),
    );
  }
              ...
              children: [
                Expanded(
                  child: _ExportButton(
                    ...
                    sublabel: kIsWeb ? 'Tải về máy tính' : 'Zalo / Messenger / Gmail',
                ...
                Expanded(
                  child: _ExportButton(
                    ...
                    sublabel: kIsWeb ? 'Lưu & in hóa đơn' : 'In qua ứng dụng khác',
                    color: AppColors.red,
              ...
              children: [
                Row(
                  ...
                  children: [
                    Column(
                      crossAxisAlignment: CrossAxisAlignment.start,
                      children: [
                        Text('HÓA ĐƠN TIỀN PHÒNG',
                          style: GoogleFonts.manrope(fontSize: 11, fontWeight: FontWeight.w700, color: Colors.white70, letterSpacing: 1)),
                ...
                Row(
                  children: [
                    ...
                    if (tenant != null) _HeaderChip(Icons.person_outline, tenant!.name),
                  ],
                ),
              ],
            ),
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                // Property info
                if (property != null) ...[
                  _InfoRow(Icons.apartment_outlined, property!.name, AppColors.textSecondary),
                  if (property!.address.isNotEmpty)
                    _InfoRow(Icons.location_on_outlined, property!.address, AppColors.textTertiary),
                  const SizedBox(height: 16),
                ],
                ...
                _LineItem('Tiền phòng', rentPrice, icon: Icons.bed_outlined, highlight: true),
                if (services.isNotEmpty) ...[
                  const SizedBox(height: 8),
                  ...services.map((s) => Padding(
                    padding: const EdgeInsets.only(bottom: 8),
                    child: _LineItem(s.name, s.price, icon: Icons.miscellaneous_services_outlined),
                  )),
                ],
                ...
                _LineItem(
                  ...
                  electricCost,
                  subtitle: '${(electricNew - electricOld).clamp(0, 99999)} kWh × ${_fmt(electricPrice)}',
                  icon: Icons.bolt,
                ),
                const SizedBox(height: 8),
                _LineItem(
                  ...
                  waterCost,
                  subtitle: '${(waterNew - waterOld).clamp(0, 99999)} m³ × ${_fmt(waterPrice)}',
                  ...
                  child: Row(
                    ...
                    children: [
                      Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('TỔNG CỘNG', style: GoogleFonts.manrope(fontSize: 11, fontWeight: FontWeight.w700, color: AppColors.primary)),
                      ...
                      Text(_fmt(total),
                        style: GoogleFonts.manrope(fontSize: 26, fontWeight: FontWeight.w800, color: AppColors.primary)),
                    ],
                  ),
                ),
                ...
                const SizedBox(height: 16),
                Center(
                  child: Text('Phần mềm Quản lý Nhà Trọ - Azure Clarity',
    ...
    return Container(
      ...
      child: Row(
        ...
        children: [
          ...
          Text(label, style: GoogleFonts.manrope(fontSize: 12, fontWeight: FontWeight.w600, color: Colors.white)),
        ],
      ),
    );
  }
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const SizedBox(width: 6),
          Expanded(child: Text(text, style: GoogleFonts.manrope(fontSize: 13, color: color))),
        ],
      ),
    );
  }
...
class _SectionLabel extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Text(label,
      style: GoogleFonts.manrope(fontSize: 10, fontWeight: FontWeight.w800, letterSpacing: 0.8, color: AppColors.textTertiary));
  }
}
...
class _LineItem extends StatelessWidget {
  ...
  const _LineItem(this.label, this.value, {this.subtitle, this.icon, this.highlight = false});
        ...
        Expanded(
          child: Column(
            ...
            children: [
              ...
              if (subtitle != null)
                Text(subtitle!, style: GoogleFonts.manrope(fontSize: 11, color: AppColors.textTertiary)),
            ],
          ),
        ),
        Text(_fmt(value),
...
class _ExportButton extends StatelessWidget {
  ...
  const _ExportButton({required this.icon, required this.label, required this.sublabel, required this.color, this.onTap});
        ...
        child: Container(
          ...
          child: Column(
            children: [
              ...
              Text(sublabel, style: GoogleFonts.manrope(fontSize: 11, color: AppColors.textSecondary), textAlign: TextAlign.center),
            ],
          ),
        ),
      ),
